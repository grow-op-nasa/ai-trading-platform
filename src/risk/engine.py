"""Position sizing -- src/risk.
    ...
    from src.risk import PositionSizer, RiskLimits, AccountState

    sizer = PositionSizer(RiskLimits(risk_per_trade_pct=0.10, max_portfolio_exposure_pct=0.5))
...
uses a fixed fraction of account equity per trade, applied identically
regardless of `Signal.confidence` -- no confidence-scaling in this
first version.
"""
...
class PositionSizer:
    ...
    def size(self, signal: Signal, account: AccountState, price: float) -> SizingDecision:
        ...
        Returns:
            ...
            at the full `risk_per_trade_pct` of equity. `LONG` and
        ...
        desired_capital = account.equity * self._limits.risk_per_trade_pct
        ...
        if allocated_capital < desired_capital:
            ...
            reason = (
                f"sized at full per-trade risk "
                f"({self._limits.risk_per_trade_pct:.1%} of equity)"
            )
