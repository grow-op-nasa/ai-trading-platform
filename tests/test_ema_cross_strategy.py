...
def test_name_is_ema_cross():
    assert EMACrossStrategy(fast=2, slow=4).name == "ema_cross"
...
def test_prepare_does_not_mutate_input():
    ...
    EMACrossStrategy(fast=2, slow=4).prepare(candles)
...
def test_confidence_is_fixed_and_configurable():
    ...
    candles = make_candles(closes)
    strategy = EMACrossStrategy(fast=2, slow=4, confidence=0.42)
    prepared = strategy.prepare(candles)
